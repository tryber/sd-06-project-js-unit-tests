...
describe('#productDetails', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.deepStrictEqual(products.map(item => typeof (item)), ['object', 'object']);

    // Teste que os dois objetos são diferentes entre si.
    assert.notDeepStrictEqual(products[0], products[1]);

    // (Difícil) Teste que os dois productIds terminam com 123.
    const productIdsEndsWith123 = products.map(item => item.details.productId.slice(-3) === '123');
    assert.deepStrictEqual(productIdsEndsWith123, [true, true])
  });
});
