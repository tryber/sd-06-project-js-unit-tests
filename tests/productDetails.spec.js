...
describe('#productDetails', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    let final = productDetails('Alcool', 'Mascara')[0].details.productId;
    assert.strictEqual(final.substring(final.length-3, final.length), '123');
    let final2 = productDetails('Alcool', 'Mascara')[1].details.productId;
    assert.strictEqual(final2.substring(final2.length-3, final2.length), '123');
  });
});
