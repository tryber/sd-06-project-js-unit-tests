...
describe('#circle', () => {
  it('given a radius, should return an object with circles info', () => {
    ...
    assert.deepStrictEqual(parseFloat((circle(2).circumference).toPrecision(4)), 12.56);
    // Teste que a função retorna, dentro de um objeto, a circunferência correta para um círculo de raio 2.
    assert.deepStrictEqual(parseFloat((circle(3).area).toPrecision(4)), 28.26);
    // Teste que a função retorna, dentro de um objeto, a área correta para um círculo de raio 3.
    assert.deepStrictEqual({
      area: parseFloat((circle(3).area).toPrecision(4)),
      circumference: parseFloat((circle(3).circumference).toPrecision(4))
