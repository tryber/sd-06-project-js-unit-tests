...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.deepStrictEqual(typeof createMenu(myMenu), 'object');
    ...
    // ```
    const restaurant = createMenu(myMenu);
    ...
    // ```
    assert.deepStrictEqual(createMenu(myMenu).fetchMenu, restaurant.fetchMenu);
    ...
    restaurant.consumption // Retorno: ["coxinha"]
    assert.deepStrictEqual(restaurant.consumption, ['coxinha']);
    // Agora faça o PASSO 3 no arquivo `src/restaurant.js`.
    // --------------------------------------------------------------------------------------
    // TESTE 6: Verifique que as três orders seguintes, de bebidas e comidas mescladas, somam três itens no array `objetoRetornado.consumption` conforme os itens pedidos.
    // ```
    /*I broke my head trying to prevent the values posted in the previous tests from  getting in the way of the later tests. After talking with some colleagues, i received a tip do reset the consumption array before each test;
    */
    ...
    restaurant.order('agua');
    restaurant.order('coxinha');
    assert.strictEqual(Number(restaurant.pay()), 12.87);
