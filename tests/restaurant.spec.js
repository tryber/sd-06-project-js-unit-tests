...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    const meuRestaurante = { food: { coxinha: 3.9, sopa: 9.9 }, drinks: { agua: 3.9, cerveja: 6.9 } };
    assert.strictEqual(typeof createMenu(meuRestaurante), 'object');
    assert.strictEqual(typeof createMenu(meuRestaurante).fetchMenu, 'object');
    ...
    // ```
    const objVazio = { food: {}, drink: {} };
    assert.deepStrictEqual(createMenu({ food: {}, drink: {} }).fetchMenu, objVazio);
    ...
    const meuMenu = createMenu(meuRestaurante);
    meuMenu.order('coxinha');
    meuMenu.order('agua');
    meuMenu.order('coxinha');
    // meuMenu.comsuption // Retorno: ['coxinha', 'agua', 'coxinha']

    assert.deepStrictEqual(meuMenu.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    // ```
    const novoObjeto = createMenu(meuRestaurante);
    novoObjeto.order('coxinha');
    novoObjeto.order('agua');
    novoObjeto.order('coxinha');
    // novoObjeto.pay(); // Retorno: somaDosPreçosDosPedidos
    assert.strictEqual(novoObjeto.pay(), 3.90 * 3);
    // ```
