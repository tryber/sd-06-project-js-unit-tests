...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    assert.strictEqual(createMenu(objParameter).fetchMenu, objParameter);
    ...
    // ```
    assert.deepStrictEqual(Object.keys(createMenu(objParameter).fetchMenu), ['food', 'drinks']);
    // Agora faça o TESTE 3 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    const objetoRetornado = createMenu(objParameter);
    // objetoRetornado.fetchMenu // Retorno: objetoQualquer
    // ```
    assert.strictEqual(objetoRetornado.fetchMenu, objParameter);
    ...
    const objetoRetornado2 = createMenu(objParameter);
     objetoRetornado2.order('coxinha');
     objetoRetornado2.order('agua');
     objetoRetornado2.order('coxinha');
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    assert.deepStrictEqual(objetoRetornado2.pay(), 12.87);
