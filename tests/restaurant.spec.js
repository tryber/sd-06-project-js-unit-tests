...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const objetoRestaurante = { food: { coxinha: 3.90, sanduiche: 9.90 }, drinks: { agua: 3.90, cerveja: 6.90 },};
    // ```
    // createMenu(objetoQualquer) // Retorno: { fetchMenu: objetoQualquer }
    const meuRestaurante = createMenu(objetoRestaurante);
    assert.deepStrictEqual(meuRestaurante.fetchMenu(), objetoRestaurante);
    // ```
    ...
    assert.deepStrictEqual(Object.keys(meuRestaurante.fetchMenu()), ['food', 'drinks']);
    // ```
    ...
    const restaurante = createMenu(objetoRestaurante);
    ...
    const preco = (objetoRestaurante.food.coxinha * 2) + objetoRestaurante.drinks.agua;
    assert.strictEqual(restaurante.pay(), preco);
    // ```
    // objetoRetornado.order('coxinha');
    // objetoRetornado.order('agua');
    // objetoRetornado.order('coxinha');
