...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    let objReturned =  createMenu(obj);
    assert.deepEqual(Object.entries(objReturned)[0], ['fetchMenu', obj]);
    ...
    obj = { food: {}, drink: {} };
    objReturned =  createMenu(obj);
    assert.deepEqual(Object.keys(objReturned.fetchMenu), ['food', 'drink']);
    ...
    assert.deepEqual(objReturned.fetchMenu, { food: {}, drink: {} });  
    ...
    assert.deepEqual(objReturned.consumption, []);  
    ...
    objReturned.order('coxinha');
    assert.deepEqual(objReturned.consumption, ['coxinha']);
    ...
    objReturned.order('agua');
    objReturned.order('sopa');
    objReturned.order('sashimi');
    assert.deepEqual(objReturned.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);
    // ```
    // objetoRetornado.order("coxinha");
    ...
    obj = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} };
    objReturned =  createMenu(obj);
    objReturned.order('coxinha');
    objReturned.order('agua');
    objReturned.order('coxinha');
    assert.deepEqual(objReturned.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    assert.strictEqual(objReturned.pay(), 11.70);
