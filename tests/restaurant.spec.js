...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    assert.deepStrictEqual(typeof createMenu({ food: { coxinha: 3.90, sanduiche: 9.90 } }), 'object');
    ...
    assert.deepStrictEqual(menu.fetchMenu, { food: {}, drink: {} });
    ...
    // ```
    const menuAlpha = createMenu({ food: { Xinfarto: 13.90, Refri: 5.65 } });
    assert.deepStrictEqual(menuAlpha.fetchMenu, { food: { Xinfarto: 13.90, Refri: 5.65 } });
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    // ```
    const menuBeta = createMenu({ food: { Xinfarto: 13.90, refri: 5.65 } });
    assert.deepStrictEqual(menuBeta.consumption, []);
    ...
    // ```
    const menuOmega = createMenu({ food: { Xinfarto: 13.90, refri: 5.65 } });
    menuOmega.order('coxinha');
    assert.deepStrictEqual(menuOmega.consumption, ['coxinha']);
    ...
    menuDelta.order('sashimi');
    assert.deepStrictEqual(menuDelta.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    // ```
    const menuPhi = createMenu({ food: { coxinha: 3.90, sashimi: 19.65, sopa: 15.20 }, drink: { agua: 2.90, cerveja: 7.31 } });
    menuPhi.order('coxinha');
    menuPhi.order('agua');
    menuPhi.order('coxinha');
    assert.deepStrictEqual(menuPhi.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    // ```
    const checkMenu = createMenu({ food: { coxinha: 3.90, sashimi: 19.65, sopa: 15.20 }, drink: { agua: 2.90, whisky: 170.31 } });
