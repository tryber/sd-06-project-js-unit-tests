...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {    
    ...
    const {fetchMenu} = createMenu(obj);
    assert.deepEqual(fetchMenu, obj);
    // ```
    ...
    // ```
    const objeto = createMenu(obj);
    objeto.order('coxinha');
    assert.deepEqual(objeto.consumption, ['coxinha']);
    // Agora faça o PASSO 3 no arquivo `src/restaurant.js`.
    // --------------------------------------------------------------------------------------
    // TESTE 6: Verifique que as três orders seguintes, de bebidas e comidas mescladas, somam três itens no array `objetoRetornado.consumption` conforme os itens pedidos.
    // ```
    let objetoRetornado = createMenu(obj);
    objetoRetornado.order("coxinha");
    ...
    // ```
    assert.deepEqual(objetoRetornado.consumption, ["coxinha", "agua", "sopa", "sashimi"]);
    ...
    // ```
    objetoRetornado = createMenu(obj);
    objetoRetornado.order("coxinha");
    objetoRetornado.order("agua");
    objetoRetornado.order("coxinha");
    ...
    ultimoObjeto.order('coxinha');
    ultimoObjeto.order('agua');
    ultimoObjeto.order('agua');
    assert.strictEqual(ultimoObjeto.pay(), 17.16);
