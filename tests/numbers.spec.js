...
describe('#numbers', () => {
  it('should return an array and return if it has only numbers or not', () => {
    assert.strictEqual(numbers([1, 2, 3, 4, 5]), true, 'ok');
    assert.strictEqual(numbers([1, 2, '3', 4, 5]), false, 'ok');
    assert.strictEqual(numbers([1, 'a', 3]), false, 'ok');
    assert.strictEqual(numbers([' ']), false, 'ok');
  });
