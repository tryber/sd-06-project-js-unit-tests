...
function orderFromMenu(request) {
  if (typeof request === 'string') this.consumption.push(request);
}
...
const createMenu = obj => ({
  ...
  pay() {
    ...
    if (menu.consumption.length > 0) {
      menu.consumption.forEach((element) => {
        switch (element) {
          case 'agua' || 'cerveja':
            somaDosPreçosDosPedidos += menu.fetchMenu.drinks[element];
            break;
          case 'coxinha' || 'sanduiche':
            somaDosPreçosDosPedidos += menu.fetchMenu.food[element];
            break;
          default:
            break;
        }
